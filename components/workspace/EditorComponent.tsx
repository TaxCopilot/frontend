...
import {
  ...
  ChevronDown, Minus, Plus, Type, ZoomIn, ZoomOut,
  ArrowLeft,
} from 'lucide-react';
...
export function EditorComponent({
  ...
  const baseScale = windowWidth < 800 ? (windowWidth - 32) / 794 : 1;
  const effectiveZoom = zoomLevel * baseScale;
  ...
  return (
    <div className="flex-1 flex flex-col p-4 pt-16 md:p-6 md:pt-6 overflow-hidden max-w-full relative">
      {/* ── ProseMirror CSS ── */}
      <style>{`
      ...
      <div className="bg-surface-light border border-border-default rounded-2xl shadow-card flex flex-col h-full overflow-hidden">
        ...
        <div className="border-b border-border-subtle px-3 py-2 flex flex-wrap items-center gap-1 bg-background-light/50 min-h-[50px]">
          <button 
            type="button"
            onClick={() => router.back()} 
            className="p-1.5 mr-1 rounded-lg text-text-sub hover:bg-border-subtle hover:text-text-main transition-colors flex items-center justify-center"
            title="Go Back"
          >
          ...
          <div className="flex items-center border border-border-default rounded-lg overflow-visible bg-white flex-shrink-0">
            <button type="button" title="Decrease size"
              ...
              className="px-1.5 py-1.5 hover:text-text-main hover:scale-110 transition-all text-text-sub">
          ...
          <div className="flex items-center border border-border-default rounded-lg overflow-visible bg-white flex-shrink-0">
            <button type="button" title="Zoom Out"
              ...
              className="px-2 py-1.5 hover:text-text-main hover:scale-110 transition-all text-text-sub flex items-center gap-1">
              <ZoomOut className="w-3.5 h-3.5" />
            </button>
            <div className="px-2 py-1.5 text-[12px] font-medium text-text-main min-w-[48px] text-center border-x border-border-default">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden w-full scrollbar-thin bg-[#FAF9F5] pt-8 pb-32 tiptap-wrap relative" style={{ overflowX: 'hidden', WebkitOverflowScrolling: 'touch', touchAction: 'pan-y', overscrollBehavior: 'contain' }}>
          <div 
            ...
            style={{ 
              ...
              width: windowWidth < 800 ? `${794 * effectiveZoom}px` : '100%',
              minWidth: windowWidth < 800 ? 'auto' : '794px',
              transformOrigin: 'top center',
              maxWidth: '100vw'
            }}
          >
            <div className="bg-transparent mx-auto" style={{ width: 794 }}>
